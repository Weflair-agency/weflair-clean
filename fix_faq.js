...
const newFaqList = <div class="weflair-faq__list">
...
// 1. Target exact CSS
if (html.includes('gap: 4rem;')) {
    console.log("CSS already updated in previous steps.");
} else {
    const cssTarget = '.weflair-faq__list {\\n          max-width: 54rem;\\n          margin-top: clamp(2.35rem, 3.9vw, 3.2rem)\\n        }';
    const cssReplace = \.weflair-faq__list {
          max-width: 72rem;
          margin-top: clamp(2.35rem, 3.9vw, 3.2rem);
          display: flex;
          gap: 4rem;
    ...
    html = html.replace(cssTarget, cssReplace);
}

// 2. Replace HTML
const startTag = '<div class="weflair-faq__list">';
const startIdx = html.indexOf(startTag);

if (startIdx !== -1) {
    const sectionEndIdx = html.indexOf('</section>', startIdx);
    if (sectionEndIdx !== -1) {
        // Look backwards from </section> to find the wrapping div closures.
        // We know the old structure ends with:
        //       </div>
        //     </div>
        //   </section>
        
        const finalCloseDivs = '</div>\\n        </div>\\n      </section>';
        const endIdx = html.indexOf(finalCloseDivs, startIdx);
        
        if (endIdx !== -1) {
            html = html.substring(0, startIdx) + newFaqList + "\\n      " + html.substring(endIdx + '</div>\\n        </div>\\n      '.length);
            fs.writeFileSync(path, html, 'utf8');
            console.log("SUCCESS: Replaced FAQ section and stripped out old questions.");
        } else {
            console.log("ERROR: Couldn't find exact end tag, doing a fallback replacement.");
            // Fallback: replace everything between <div class="weflair-faq__list"> and </section> minus the wrapper closure
            html = html.substring(0, startIdx) + newFaqList + "\\n        </div>\\n      " + html.substring(sectionEndIdx);
            fs.writeFileSync(path, html, 'utf8');
             console.log("SUCCESS: Fallback replacement completed.");
        }
    }
} else {
    console.log("ERROR: FAQ list start not found. It might have already been updated?");
}
