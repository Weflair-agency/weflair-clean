...
const targetStart = '<div class="weflair-faq__list">';
const targetEnd = '</section>';

const startIdx = html.lastIndexOf(targetStart);
const endIdx = html.indexOf(targetEnd, startIdx);
...
`;
    ...
    const newHtml = html.substring(0, startIdx) + newFaqList + "\n      " + html.substring(endIdx);
    ...
    if (finalHtml.includes('gap: 4rem;')) {
        console.log('CSS already replaced');
    } else {
        if (finalHtml.includes(cssTarget)) {
            finalHtml = finalHtml.replace(cssTarget, cssReplace);
            console.log('CSS block found and replaced.');
        } else {
            console.log('Could not find exact CSS string. Doing regex fallback.');
            finalHtml = finalHtml.replace(/\.weflair-faq__list\s*\{[^}]+\}/, cssReplace);
        }
    }
    
    fs.writeFileSync('index.html', finalHtml, 'utf8');
    console.log('SUCCESS: HTML Replaced via CJS');
} else {
    console.log('ERROR: tags not found, startIdx:', startIdx, 'endIdx:', endIdx);
}
