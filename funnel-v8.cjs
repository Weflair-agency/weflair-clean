...
// ─── Build new SVG inner content ─────────────────────────────────────────────
// KEY CHANGE: Recycling arrows centered at x=490 so LEFT half is BEHIND funnel,
// RIGHT half extends past. Funnel drawn AFTER arrows = funnel is ON TOP.
// Bigger scale (6.5x) to match reference proportions.
// Growth Loops box moved to far right (~x=850).

const NEW_SVG_INNER = `
<defs>
<filter id="ag"><feGaussianBlur stdDeviation="8" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
...
<!-- ═══ RECYCLING ARROWS — drawn FIRST = BEHIND funnel stages ═══ -->
<!-- Canva 4-arrow recycling pinwheel, scaled 6.5x, centered at (490, 286) -->
<!-- This makes the left ~40% sit behind the funnel, right ~60% extends past it -->
<g transform="translate(490, 286) scale(6.5) translate(-84.621, -40.031)" filter="url(#ag)">
  <!-- Arrow 1: right-center to bottom -->
  <path d="M 101.476562 50.785156 L 102.59375 56.441406 L 107.347656 54 C 99.851562 64.765625 89.605469 69.117188 75.957031 65.507812 L 75.039062 67.863281 L 70.132812 58.972656 L 79.921875 56.917969 L 78.789062 59.355469 C 87.507812 62.082031 94.5625 59.082031 101.476562 50.785156 Z" fill="#3eff68" fill-opacity="0.88" fill-rule="evenodd"/>
  <!-- Arrow 2: bottom to left -->
  <path d="M 73.871094 56.886719 L 68.210938 58 L 70.65625 62.757812 C 59.886719 55.261719 55.535156 45.015625 59.144531 31.367188 L 56.792969 30.449219 L 65.679688 25.539062 L 67.734375 35.332031 L 65.300781 34.199219 C 62.570312 42.917969 65.570312 49.972656 73.871094 56.886719 Z" fill="#3eff68" fill-opacity="0.88" fill-rule="evenodd"/>
  <!-- Arrow 3: left to top -->
  <path d="M 67.769531 29.277344 L 66.65625 23.621094 L 61.902344 26.0625 C 69.394531 15.296875 79.640625 10.945312 93.289062 14.554688 L 94.207031 12.203125 L 99.117188 21.089844 L 89.324219 23.144531 L 90.457031 20.710938 C 81.738281 17.980469 74.6875 20.980469 67.769531 29.277344 Z" fill="#3eff68" fill-opacity="0.88" fill-rule="evenodd"/>
  <!-- Arrow 4: top to right-center -->
  <path d="M 95.375 23.179688 L 101.03125 22.0625 L 98.589844 17.308594 C 109.359375 24.804688 113.707031 35.046875 110.097656 48.699219 L 112.453125 49.613281 L 103.566406 54.523438 L 101.507812 44.730469 L 103.945312 45.863281 C 106.671875 37.148438 103.671875 30.09375 95.375 23.179688 Z" fill="#3eff68" fill-opacity="0.88" fill-rule="evenodd"/>
</g>

<!-- ═══ FUNNEL STAGES — drawn SECOND = ON TOP of arrows ═══ -->
...
<!-- ═══ GROWTH LOOPS LABEL — far right, separate from arrows ═══ -->
<rect x="820" y="250" width="120" height="64" rx="10" fill="rgba(14,17,14,0.65)" stroke="rgba(62,255,104,0.30)" stroke-width="1.5"/>
<text x="880" y="278" font-family="'Space Grotesk',sans-serif" font-size="15" font-weight="700" fill="#f6f3ee" fill-opacity="0.85" text-anchor="middle">Growth</text>
<text x="880" y="300" font-family="'Space Grotesk',sans-serif" font-size="15" font-weight="700" fill="#f6f3ee" fill-opacity="0.85" text-anchor="middle">Loops</text>
`;
...
console.log('✓ Updated — arrows now OVERLAP the funnel (left half behind, right half past)');
console.log('  • Recycling arrows centered at x=490 (right edge of funnel)');
console.log('  • Scale bumped to 6.5x');
console.log('  • Left ~40% of circle is behind funnel stages');
console.log('  • Right ~60% extends past funnel');
console.log('  • Growth Loops label moved to far right (x=820)');
